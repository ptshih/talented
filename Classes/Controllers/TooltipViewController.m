...
@synthesize addButton, delButton;
@synthesize bgImage, bgTop, bgBottom;
@synthesize nameLabel, rankLabel, reqsLabel, req2Label, TLLabel, TRLabel, BLLabel, BRLabel;

- (void)viewDidLoad {
  [super viewDidLoad];
	
	_tooltip.font = [UIFont systemFontOfSize:14];
	_tooltip.textColor = [UIColor colorWithRed:0.9 green:0.746 blue:0.082 alpha:1.0];
}

// info that only has to be set once for selected button
- (void)setInfo {
	nameLabel.text = self.talentView.talent.talentName;
...
//	}
  ...
  //	NSLog(@"height so far: %d",height);
	...
	// Description size
	CGSize descSize = [_tooltip.text sizeWithFont:[UIFont systemFontOfSize:14]
                                      constrainedToSize:CGSizeMake(226, 1000)
...
//	[data addPoint:button];
//	[self updateInfo];
	//	NSLog(@"infopage add: r: %i, c: %i, rank: %i",button.row, button.col, button.rank);
}

- (IBAction)removePoint:(id)sender {
  [self.view removeFromSuperview];
//	[data removePoint:button];
//	[self updateInfo];
	//	NSLog(@"info remove: r: %i, c: %i, rank: %i",button.row, button.col, button.rank);
}
